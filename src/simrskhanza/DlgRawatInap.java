...
public final class DlgRawatInap extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        lblPendingValidasi.setText("SOAP Belum Divalidasi Dokter: 0");
...
private widget.Label lblPendingValidasi;
                    ...
                    if(KdPeg.getText().trim().equals("")||TPegawai.getText().trim().equals("")){
                        ...
                        if(akses.getkode().equals("Admin Utama")){
                            if(Sequel.menyimpantf("pemeriksaan_ranap","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","Data",24,new String[]{
                                    ...
                                    TPenilaian.getText(),TindakLanjut.getText(),TInstruksi.getText(),TEvaluasi.getText(),statusVerif,tglVerif,nipVerif,null,KdPeg.getText()
                                })==true){
                                tabModePemeriksaan.addRow(new Object[]{
    ...
    private void validasiSOAPOlehDokter() {
        ...
        if (konfirm == JOptionPane.YES_OPTION) {
            String sql = "UPDATE pemeriksaan_ranap SET " +
    ...
    private void tampilkanPendingValidasi() {
        try {
            ...
            try {
                ...
                if (!noRaw.equals("")) {
                    jml = Integer.parseInt(Sequel.cariIsi("SELECT COUNT(*) FROM pemeriksaan_ranap WHERE verifikasi='Belum' AND no_rawat=?", noRaw));
                }
            } catch (Exception ex) {
                jml = 0;
            }
