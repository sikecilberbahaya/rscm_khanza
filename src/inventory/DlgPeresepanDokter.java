...
public final class DlgPeresepanDokter extends javax.swing.JDialog {
    ...
    private WarnaTableResep warna=new WarnaTableResep();
    private WarnaTable2 warna2=new WarnaTable2();
    private WarnaTableResep warna3=new WarnaTableResep();
    ...
    private String pilihiterasi="",noracik="",aktifkanbatch="no",STOKKOSONGRESEP="no",qrystokkosong="",status="",bangsal="",resep="",DEPOAKTIFOBAT="",
            kamar="",norawatibu="",kelas,RESEPRAJALKEPLAN="no",NOTIFMAKSIMALNOMINALRESEPRAJAL="no",AKTIFBATASHARGABPJS="NO";
    private double BATASHARGABPJS=100000;
    private File file;
    ...
    public DlgPeresepanDokter(java.awt.Frame parent, boolean modal) {
        ...
        try {
            AKTIFBATASHARGABPJS=koneksiDB.AKTIFBATASHARGABPJS();
        } catch (Exception e) {
            AKTIFBATASHARGABPJS="yes";
        }
        
        try {
            BATASHARGABPJS=Double.parseDouble(koneksiDB.BATASHARGABPJS());
        } catch (Exception e) {
            BATASHARGABPJS=100000;
        }
    ...
    private void initComponents() {
        ...
        cmbIterasi.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Tanpa Iterasi", "Iterasi 1 Kali", "Iterasi 2 Kali" }));
        cmbIterasi.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        cmbIterasi.setName("cmbIterasi"); // NOI18N
        cmbIterasi.setPreferredSize(new java.awt.Dimension(120, 23));
        cmbIterasi.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(java.awt.event.ItemEvent evt) {
                cmbIterasiItemStateChanged(evt);
            }
        });
        ...
        cmbIterasi.setBounds(710, 70, 120, 23);
...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            TCari.requestFocus();
        }else if(!cekBatasHargaBPJS()){
            TCari.requestFocus();
        }else if(!cekObatKronis30Hari()){
            TCari.requestFocus();
        }else{
            ...
            if (reply == JOptionPane.YES_OPTION) {                 
                ...
                if(ubah==false){
                    if(Sequel.menyimpantf2("resep_obat","?,?,?,?,?,?,?,?,?,?,?","Nomer Resep",11,new String[]{
                        ...
                        cmbJam.getSelectedItem()+":"+cmbMnt.getSelectedItem()+":"+cmbDtk.getSelectedItem(),status,"0000-00-00","00:00:00", cmbIterasi.getSelectedItem().toString()
                        })==true){
                            simpandata();
                    }else{
                        emptTeksobat2();
                    ...
                    // Update iterasi mulai
                    String iterasi = cmbIterasi.getSelectedItem().toString();
                    if (!iterasi.equals("Tanpa Iterasi") && !iterasi.equals("Iterasi 1 Kali") && !iterasi.equals("Iterasi 2 Kali")) {
                        JOptionPane.showMessageDialog(null, "Pilihan iterasi tidak valid");
                        Sequel.AutoComitTrue();
                        return;
                    }
                    // Update iterasi selesai
                    Sequel.mengedit("resep_obat", "no_resep=?", "kd_dokter=?, tgl_peresepan=?, jam_peresepan=?, iterasi=?", 5, new String[]{
                        KdDokter.getText(),
                        Valid.SetTgl(DTPBeri.getSelectedItem() + ""),
                        cmbJam.getSelectedItem() + ":" + cmbMnt.getSelectedItem() + ":" + cmbDtk.getSelectedItem(),
                        cmbIterasi.getSelectedItem().toString(),
                        NoResep.getText()
                    });
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    */
        ...
        try {
            ...
            if(kenaikan>0){
                ...
                try{
                    ...
                    // Tambahkan setelah kode yang sudah ada
                    String iterasiDb = Sequel.cariIsi("select iterasi from resep_obat where no_resep=?", no_resep);
                    if(!iterasiDb.equals("")){
                        cmbIterasi.setSelectedItem(iterasiDb);
                    } else {
                        cmbIterasi.setSelectedItem("Tanpa Iterasi");
                    }
                }catch(Exception e){
                    System.out.println("Notifikasi Tampil Ubah : "+e);
                }finally{
    ...
    private void simpandata() {
        try {
            // Ambil nilai iterasi langsung dari combobox
             String iterasi = cmbIterasi.getSelectedItem().toString();
    ...
    public boolean cekObatKronis30Hari() {
        boolean adaObatKronis = false;
        StringBuilder pesanKronis = new StringBuilder();
        
        // Cek apakah resep saat ini mengandung obat kronis
        boolean resepIniAdaKronis = false;
        for(int idx=0; idx<tbResep.getRowCount(); idx++) {
            try {
                if(Double.parseDouble(tbResep.getValueAt(idx,1).toString()) > 0) {
                    String kodeBarang = tbResep.getValueAt(idx,3).toString();
                    String kategori = Sequel.cariIsi("select kode_kategori from databarang where kode_brng=?", kodeBarang);
                    if(kategori.toLowerCase().contains("krn")) {
                        resepIniAdaKronis = true;
                        break;
                    }
                }
            } catch (Exception e) {
        ...
        // Cek juga di racikan
        if(!resepIniAdaKronis) {
            for(int idx=0; idx<tbDetailResepObatRacikan.getRowCount(); idx++) {
                if(Valid.SetAngka(tbDetailResepObatRacikan.getValueAt(idx,13).toString()) > 0) {
                    try {
                        String kodeBarang = tbDetailResepObatRacikan.getValueAt(idx,1).toString();
                        String kategori = Sequel.cariIsi("select kode_kategori from databarang where kode_brng=?", kodeBarang);
                        if(kategori.toLowerCase().contains("krn")) {
                            resepIniAdaKronis = true;
                            break;
        ...
        // Jika resep ini tidak ada obat kronis, langsung lanjut
        if(!resepIniAdaKronis) {
            return true;
        }
        
        // Cari riwayat resep kronis 30 hari terakhir untuk pasien ini
        String noRkmMedis = Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText());
        
        PreparedStatement psKronis = null;
        ...
        try {
            psKronis = koneksi.prepareStatement(
                ...
                + "and db.kode_kategori like '%krn%' "
                + "and ro.tgl_peresepan between date_sub(curdate(), interval 30 day) and curdate() "
                + "group by ro.no_resep, ro.tgl_peresepan, d.nm_dokter "
                + "order by ro.tgl_peresepan desc");
            psKronis.setString(1, noRkmMedis);
            rsKronis = psKronis.executeQuery();
            ...
            // Cek juga dari racikan
            if(psKronis != null) { psKronis.close(); psKronis = null; }
            ...
            psKronis = koneksi.prepareStatement(
                ...
                + "inner join resep_dokter_racikan_detail rdrd on rdr.no_resep=rdrd.no_resep and rdr.no_racik=rdrd.no_racik "
                + "inner join databarang db on rdrd.kode_brng=db.kode_brng "
                + "inner join dokter d on ro.kd_dokter=d.kd_dokter "
                + "where rp.no_rkm_medis=? "
                + "and db.kode_kategori like '%krn%' "
                + "and ro.tgl_peresepan between date_sub(curdate(), interval 30 day) and curdate() "
                + "and ro.no_resep not in (select rd2.no_resep from resep_dokter rd2 inner join databarang db2 on rd2.kode_brng=db2.kode_brng where db2.kode_kategori like '%krn%') "
                + "group by ro.no_resep, ro.tgl_peresepan, d.nm_dokter "
                + "order by ro.tgl_peresepan desc");
            psKronis.setString(1, noRkmMedis);
            rsKronis = psKronis.executeQuery();
            ...
            System.out.println("Notifikasi cek obat kronis : " + e);
        } finally {
            try { if(rsKronis != null) rsKronis.close(); } catch (Exception e) {}
            try { if(psKronis != null) psKronis.close(); } catch (Exception e) {}
        }
        
        if(adaObatKronis) {
            int reply = JOptionPane.showConfirmDialog(rootPane,
                "PERHATIAN! Pasien ini sudah pernah mendapatkan obat KRONIS dalam 30 hari terakhir:\n\n" 
                + pesanKronis.toString() 
                + "\nApakah Anda yakin tetap ingin melanjutkan peresepan obat kronis?",
                "Peringatan Obat Kronis", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
    ...
    public boolean cekBatasHargaBPJS() {
        // Cek apakah fitur pembatasan harga BPJS diaktifkan
        if(!"yes".equals(AKTIFBATASHARGABPJS)) {
            return true; // Jika tidak aktif, langsung return true (lanjutkan)
        }
        
        // Cek apakah jenis bayar pasien adalah BPJS
        String namaPenjamin = Sequel.cariIsi("select png_jawab from penjab where kd_pj=?", KdPj.getText());
        
        // Cek apakah mengandung kata "BPJS" (case insensitive)
        if (namaPenjamin.toUpperCase().contains("BPJS")) {
            double totalNonKronis = 0;
            
            // Hitung total dari obat resep umum (non-kronis)
            for(int idx=0; idx<tbResep.getRowCount(); idx++) {
                try {
                    if(Double.parseDouble(tbResep.getValueAt(idx,1).toString()) > 0) {
                        String kodeBarang = tbResep.getValueAt(idx,3).toString();
                        ...
                        // Jika bukan kronis, tambahkan ke total
                        if(!kategori.toLowerCase().contains("krn")) {
                            double harga = Double.parseDouble(tbResep.getValueAt(idx,7).toString());
                            double jumlah = Double.parseDouble(tbResep.getValueAt(idx,1).toString());
                            totalNonKronis += Math.round(jumlah * harga);
                        }
                    }
                } catch (Exception e) {
            ...
            // Hitung total dari obat racikan (non-kronis)
            for(int idx=0; idx<tbDetailResepObatRacikan.getRowCount(); idx++) {
                if(Valid.SetAngka(tbDetailResepObatRacikan.getValueAt(idx,13).toString()) > 0) {
                    try {
                        String kodeBarang = tbDetailResepObatRacikan.getValueAt(idx,1).toString();
                        ...
                        // Jika bukan kronis, tambahkan ke total
                        if(!kategori.toLowerCase().contains("krn")) {
                            double harga = Double.parseDouble(tbDetailResepObatRacikan.getValueAt(idx,4).toString());
                            double jumlah = Double.parseDouble(tbDetailResepObatRacikan.getValueAt(idx,13).toString());
                            totalNonKronis += Math.round(jumlah * harga);
                        }
                    } catch (Exception e) {
                        // Skip jika error parsing
            ...
            // Cek apakah total non-kronis melebihi batas
            if (totalNonKronis > BATASHARGABPJS) {
                JOptionPane.showMessageDialog(null, 
                    "Maaf, total resep obat non-kronis untuk pasien BPJS tidak boleh melebihi Rp " + Valid.SetAngka(BATASHARGABPJS) + "!\n" +
                    "Total obat non-kronis saat ini: Rp " + Valid.SetAngka(totalNonKronis),
                    "Batas Harga Terlampaui", 
    ...
    private void cmbIterasiItemStateChanged(java.awt.event.ItemEvent evt) {
    }
    
    private void cmbIterasiKeyPressed(java.awt.event.KeyEvent evt) {
    }
}
