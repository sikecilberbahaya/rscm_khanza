...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            pcra_icra_persyaratan_harus_dipenuhi=false,satu_sehat_kirim_questionresponse_telaah_farmasi=false,satu_sehat_kirim_allergy_intolerance=false,satu_sehat_kirim_episode_of_care=false,konsultasi_perawat=false,jawaban_konsultasi_perawat=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.satu_sehat_kirim_allergy_intolerance,user.satu_sehat_kirim_episode_of_care,user.konsultasi_perawat,user.jawaban_konsultasi_perawat from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
    ...
    private void setTampil2(){
        ...
        if("[L]Kirim Episode Of Care Satu Sehat".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[L]Kirim Episode Of Care Satu Sehat",satu_sehat_kirim_episode_of_care});
        }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[L]Kirim Episode Of Care Satu Sehat".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","satu_sehat_kirim_episode_of_care='"+tbUser.getValueAt(i,2).toString()+"'");
            }
