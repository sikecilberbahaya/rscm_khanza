...
public class koneksiDB {
    ...
    private static void reconnect() throws SQLException {
        ...
        while (retries > 0) {
            try {
                ...
                System.out.println(
                    "  Koneksi Berhasil. on oadin \n\n"+
                    "  SIMRS 0.1.1 for Medical Record Electronic based on Khanza HMS\n" +
                    "                                                                     \n" +
                    "  Jika terjadi error segera hubungi petugas berwenang \n" +
                    "  pelaporan disertakan dengan SCREENSHOOT tampilan ini, dan tampilan SIMRS \n" +
                    "                                                                           \n"+
                    "                                                                           \n"+
                    "  Setiap petugas bertanggung jawab dengan USER ID dan Password masing-masing \n" +
                    "  Diharapkan untuk setiap user merubah passwordnya setiap sebulan sekali \n" +
                    "                                                                           "
                );         
                return;
    ...
    public static String AKTIFBATASHARGABPJS(){
        try (FileInputStream fis = new FileInputStream("setting/database.xml")) {
            prop.loadFromXML(fis);
            var=prop.getProperty("AKTIFBATASHARGABPJS","no");
            if(var==null) var="no";
        }catch(Exception e){
            var="no"; 
        }
    ...
    public static String BATASHARGABPJS(){
        try (FileInputStream fis = new FileInputStream("setting/database.xml")) {
            prop.loadFromXML(fis);
            var=prop.getProperty("BATASHARGABPJS","100000");
            if(var==null) var="100000";
        }catch(Exception e){
            var="100000"; 
        }
        return var;
