            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (detik.equals("01") && ((nilai_menit % 5) == 0)) {
                    ...
                    try {
                        ...
                        //pasien JKN
                        /*
                        Blok antrean/add pasien JKN (statuskirim='Belum') DINONAKTIFKAN di versi ini.
                        Biarkan apa adanya. Jika ingin diaktifkan kembali, salin dari source asli Khanza.
                         */
                        ...
                        try {
                            ...
                            while (rs.next()) {
                                ...
                                // ===== TAMBAHAN: Task ID 1 (PASIEN BARU) — versi waktu RS =====
                                if ("1".equals(rs.getString("pasienbaru")) && task1.equals("")) {
                                    int md = 0;
                                    try {
                                        md = Integer.parseInt(rs.getString("no_rawat").substring(13, 14));
                                    } catch (Exception ee) {
                                        md = 0;
                                    }
                                    md = md % 5;
                                    datajam = Sequel.cariIsi("select SUBDATE(referensi_mobilejkn_bpjs.validasi, INTERVAL " + (34 + md) + " MINUTE) from referensi_mobilejkn_bpjs where referensi_mobilejkn_bpjs.no_rawat=?", rs.getString("no_rawat"));
                                    if (!datajam.equals("") && Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid", "?,?,?", "task id", 3, new String[]{rs.getString("no_rawat"), "1", datajam})) {
                                        parsedDate = dateFormat.parse(datajam);
                                        try {
                                            TeksArea.append("Menjalankan WS taskid 1 (pasien baru) Mobile JKN Pasien BPJS\n");
                                            headers = new HttpHeaders();
                                            headers.setContentType(MediaType.APPLICATION_JSON);
                                            headers.add("x-cons-id", koneksiDB.CONSIDAPIMOBILEJKN());
                                            utc = String.valueOf(api.GetUTCdatetimeAsString());
                                ...
                                // ===== TAMBAHAN: Task ID 2 (PASIEN BARU) =====
                                if ("1".equals(rs.getString("pasienbaru")) && task1.equals("Sudah") && task2.equals("")) {
                                    int md = 0;
                                    try {
                                        md = Integer.parseInt(rs.getString("no_rawat").substring(13, 14));
                                    } catch (Exception ee) {
                                    ...
                                    md = md % 4;
                                    datajam = Sequel.cariIsi("select SUBDATE(referensi_mobilejkn_bpjs.validasi, INTERVAL " + (18 + md) + " MINUTE) from referensi_mobilejkn_bpjs where referensi_mobilejkn_bpjs.no_rawat=?", rs.getString("no_rawat"));
                                    if (!datajam.equals("") && Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid", "?,?,?", "task id", 3, new String[]{rs.getString("no_rawat"), "2", datajam})) {
                                        parsedDate = dateFormat.parse(datajam);
                                        try {
                                            TeksArea.append("Menjalankan WS taskid 2 (pasien baru) Mobile JKN Pasien BPJS\n");
                                            headers = new HttpHeaders();
                                            headers.setContentType(MediaType.APPLICATION_JSON);
                                            headers.add("x-cons-id", koneksiDB.CONSIDAPIMOBILEJKN());
                                            utc = String.valueOf(api.GetUTCdatetimeAsString());
                                ...
                                //Batas Custom
                                if (task3.equals("") && (!"1".equals(rs.getString("pasienbaru")) || task2.equals("Sudah"))) {
